...
namespace Commons.Media.PortAudio
{
	...
	{
		public PortAudioInputStream (PaStreamParameters inputParameters, double sampleRate, uint framesPerBuffer, 
                                     PaStreamFlags streamFlags, StreamCallback streamCallback, IntPtr userData)
		...
		public PortAudioInputStream (int numInputChannels, PaSampleFormat sampleFormat, double sampleRate, 
                                     uint framesPerBuffer, StreamCallback streamCallback, IntPtr userData)
	...
	{
		public PortAudioOutputStream (PaStreamParameters outputParameters, double sampleRate, 
                                      uint framesPerBuffer, PaStreamFlags streamFlags, 
                                      StreamCallback streamCallback, object userData)
			: base (outputParameters.sampleFormat, outputParameters.channelCount)
		{
			...
			try {
                using (var output = Factory.ToNative<PaStreamParameters>(outputParameters))
                    //HandleError (PortAudioInterop.Pa_OpenStream (
                    PortAudioInterop.Pa_OpenStream(
                    out handle,
                    IntPtr.Zero,
                    output.Native,
                    sampleRate,
		...
		public PortAudioOutputStream (int numOutputChannels, PaSampleFormat sampleFormat, double sampleRate,
                                      uint framesPerBuffer, object streamCallback, object userData)
                                      //  uint framesPerBuffer, StreamCallback streamCallback, object userData)
			: base (sampleFormat, numOutputChannels)
		{
			...
			try {
				HandleError (PortAudioInterop.Pa_OpenDefaultStream (
					...
					(IntPtr) framesPerBuffer,
                    null,
//                    streamCallback != null ? ToPaStreamCallback(streamCallback, true) : IntPtr.Zero,
					userData != null ? gch.AddrOfPinnedObject () : IntPtr.Zero));
			} finally {
				if (userData != null)
					gch.Free ();
	...
	public abstract class PortAudioStream : IDisposable
	{
		...
		public delegate PaStreamCallbackResult StreamCallback (byte[] buffer, int offset, int byteCount,
                           PaStreamCallbackTimeInfo timeInfo, PaStreamCallbackFlags statusFlags, IntPtr userData);
		...
		int FramesToBytes (ulong frames)
		{
			...
			case PaSampleFormat.Float32:
				return (int)((int)frames * 4 * channels);
			case PaSampleFormat.Int16:
				return (int)((int)frames * 2 * channels);
			case PaSampleFormat.Int24:
				return (int)((int)frames * 3 * channels);
			case PaSampleFormat.NonInterleaved:
			case PaSampleFormat.Int8:
			default:
				return (int)((int)frames * channels);
			}
		}
