...
# define SY_KJMP2_INTERFACE __declspec(dllexport)
#else
# define SY_KJMP2_INTERFACE __declspec(dllimport)
#endif
...
//Interfaces added by softsyst
#ifdef __cplusplus
extern "C" {
	SY_KJMP2_INTERFACE int __cdecl mp2_initialize();
	SY_KJMP2_INTERFACE int __cdecl mp2_getSampleRate(unsigned char* buf);
	SY_KJMP2_INTERFACE unsigned long __cdecl mp2_decode(unsigned char* buf, int len, short* pcm); 
}
#endif
